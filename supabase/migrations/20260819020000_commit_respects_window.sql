-- =============================================================================
-- Migration: commit_registration respeita a janela de inscricao
-- A janela vale para quem chega sem reserva viva. Quem reservou dentro do prazo
-- e enviou depois passa de proposito.
-- CREATE OR REPLACE sobre a funcao de 20260818050000, que nao pode ser editada
...
--
-- Repetido aqui, e nao herdado de 20260818050000: CREATE OR REPLACE preserva a
...
COMMENT ON FUNCTION public.commit_registration(uuid, uuid, text, jsonb, jsonb) IS
  'Grava inscricao e autoavaliacoes numa transacao, honrando a classificacao prometida pela reserva. A janela entre registration_start_date e registration_end_date (bordas inclusivas; data nula nao fecha nada) so e checada para quem chega SEM reserva viva: quem reservou dentro do prazo e enviou depois e aceito de proposito, porque a reserva e a autorizacao e recusa-lo recriaria o "pagou e foi recusado" na correria de ultima hora — a tolerancia se limita ao TTL, ja que reserve_registration_slot parou de renovar apos o prazo. Retorna {success:true, registration_id, is_waitlist} ou {success:false, reason} com reason em not_found | not_open | already_registered | reservation_expired.';
