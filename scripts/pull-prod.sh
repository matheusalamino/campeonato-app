...
SECRETS_FILE="$ROOT_DIR/.secrets/production.env"
DUMPS_DIR="$ROOT_DIR/dumps"
TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
RAW_DUMP_REL="dumps/.production_${TIMESTAMP}.raw.sql"
FINAL_DUMP_REL="dumps/production_${TIMESTAMP}.sql"
...
if ! command -v docker >/dev/null 2>&1; then
  echo "Docker nao encontrado. Instale o Docker Desktop antes de puxar producao." >&2
  exit 1
fi
...
echo "AVISO: voce esta prestes a gerar um dump de PRODUCAO e restaurar localmente."
echo "Esse fluxo so deve ser usado quando staging nao for suficiente."
printf "Digite PROD para continuar: "
read -r confirmation
...
echo "Gerando dump de producao em $FINAL_DUMP_REL..."
cd "$ROOT_DIR"
...
echo "Sanitizando dump de producao..."
...
echo "Fluxo de producao concluido."
