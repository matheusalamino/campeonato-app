...
/**
 * As posicoes chegam aqui como CODIGO (`MEI`, `GOL`), e nao como palavra.
 *
 * `radarDataFrom` recebe `form.preferred_position` do wizard, que desde a A8-T3
 * guarda o mesmo vocabulario da coluna `players.preferred_position`. Quem
 * escolher o goleiro pelo codigo e receber as habilidades de LINHA nao ve o
 * radar quebrar: ve o radar errado, calado, e grava as notas erradas.
 */
...
describe("radarDataFrom", () => {
  it("usa as habilidades de linha para jogador de linha", () => {
    expect(radarDataFrom({}, "MEI").map((p) => p.skill)).toEqual([...LINE_SKILLS]);
  });

  it("usa as habilidades de goleiro para goleiro", () => {
    expect(radarDataFrom({}, "GOL").map((p) => p.skill)).toEqual([...KEEPER_SKILLS]);
  });

  it("converte a nota de 1..5 para o dominio 0..100 do radar", () => {
    const v = porSkill(radarDataFrom({ visao: 1, controle: 3, finalizacao: 5 }, "MEI"));
...
describe("hasAnyRating", () => {
  it("e falso quando nada foi avaliado", () => {
    expect(hasAnyRating({}, "MEI")).toBe(false);
  });

  it("e verdadeiro na primeira estrela", () => {
    expect(hasAnyRating({ visao: 1 }, "MEI")).toBe(true);
  });
  ...
  it("ignora nota sobrada de outra posicao", () => {
    ...
    expect(hasAnyRating({ reflexo: 5 }, "MEI")).toBe(false);
  });
});
