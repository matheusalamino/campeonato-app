...
export default function SeasonStatsPanel({ championships }: SeasonStatsPanelProps) {
  ...
  const { rankings, loading } = usePublicRankings(selectedId || null, 10);
  ...
  return (
    <div>
      {/* Year filter — only when multiple years exist */}
      {years.length > 1 && (
      ...
      {/* Season pills — show championship names */}
      {filtered.length > 1 && (
          ...
          {filtered.map((c) => (
            <button
              ...
              style={
                c.id === selectedId
                  ? { background: "rgba(212,160,23,0.12)", border: "1px solid var(--gala-gold-2)", color: "var(--gala-gold-2)" }
                  : { background: "linear-gradient(180deg, var(--gala-panel), var(--gala-panel-2))", border: "1px solid var(--gala-line)", color: "var(--gala-ink-dim)" }
              }
            >
              {c.name}
            </button>
      ...
      <nav className="mb-0 flex flex-wrap gap-1" role="tablist">
        {TABS.map((t) => (
          <button
            ...
            style={
              tab === t.id
                ? { background: "linear-gradient(180deg, var(--gala-panel), var(--gala-panel-2))", borderColor: "var(--gala-line)" }
                : {}
            }
          >
            {t.label}
