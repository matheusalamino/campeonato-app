  ...
  if (current) {
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center gap-[2.2vh]">
        ...
        <div className="flex w-full items-start justify-center gap-3 md:gap-[3vw]">
    ...
    <div className="flex h-full flex-col items-center justify-center gap-[3.5vh]">
      ...
      {last ? (
        <div className="flex flex-col items-center gap-2">
          <span className="text-[10px] md:text-[1vw] uppercase tracking-[4px] text-[var(--gala-ink-dim)]">Último resultado</span>
          <div className="flex items-center gap-3 md:gap-[2vw]">
            <div className="flex flex-1 min-w-0 md:w-[18vw] md:flex-none flex-col items-center gap-2">
              <Crest team={last.home} />
              <span className="text-sm md:text-[1.6vw] font-extrabold text-center truncate w-full">{last.home.name.toUpperCase()}</span>
            </div>
            <ScorePanel home={last.homeScore} away={last.awayScore} />
            <div className="flex flex-1 min-w-0 md:w-[18vw] md:flex-none flex-col items-center gap-2">
              <Crest team={last.away} />
              <span className="text-sm md:text-[1.6vw] font-extrabold text-center truncate w-full">{last.away.name.toUpperCase()}</span>
            </div>
          </div>
          {last.penaltyWinner ? (
            <span className="flex items-center gap-1 md:gap-[0.6vw] rounded-full border border-[var(--gala-gold-3)]/40 bg-[var(--gala-gold-3)]/10 px-3 md:px-[1.2vw] py-1 text-xs md:text-[1.1vw] font-bold text-[var(--gala-gold-1)]">
      ...
      {next ? (
        <div className="flex flex-col items-center gap-2">
          <span className="text-[10px] md:text-[1vw] uppercase tracking-[4px] text-[var(--gala-ink-dim)]">Próximo jogo</span>
          <div className="flex items-center gap-3 md:gap-[2vw]">
            <div className="flex flex-1 min-w-0 md:w-[16vw] md:flex-none flex-col items-center gap-2">
              <Crest team={next.home} />
              <span className="text-sm md:text-[1.6vw] font-extrabold text-center truncate w-full">{next.home.name.toUpperCase()}</span>
            </div>
            <span className="text-base md:text-[1.2vw] font-light text-[var(--gala-gold-3)]">vs</span>
            <div className="flex flex-1 min-w-0 md:w-[16vw] md:flex-none flex-col items-center gap-2">
              <Crest team={next.away} />
              <span className="text-sm md:text-[1.6vw] font-extrabold text-center truncate w-full">{next.away.name.toUpperCase()}</span>
            </div>
          </div>
          {next.scheduledAt ? (
